program mpi_rbsor_3d_2
  ...
  double precision :: pi
  double precision, parameter :: omega = 1.8 ! it must be from (1, 2)

  double precision :: region_x_length, region_y_length, region_z_length
  double precision :: h_x, h_y, h_z

  double precision :: a_diag
  double precision, dimension(:, :), allocatable :: x, x_diff ! object of calc
  double precision, dimension(:, :), allocatable :: a_h_x_upper, a_h_y_upper, a_h_z_upper ! left-hand side
  double precision, dimension(:, :), allocatable :: a_h_x_lower, a_h_y_lower, a_h_z_lower ! left-hand side
  double precision, dimension(:, :), allocatable :: b ! right_hand side
  integer :: coef_h_x, coef_h_y

  ! error check
  double precision :: norm_diff_local, norm_x_local
  ...
  double precision :: tick, time0, time1, time2, time3
  double precision :: caltime, comtime, alltime
  ...
  !write(*, '(4(A,i4))') "myrank = ", myrank, ", nprocs = ", nprocs, ", start = ", start, ", goal = ", goal
  ...
  do
     ...
     !write(*, '(i5, A, i5, 2(A, e15.5))') count, ", myrank = ", myrank, &
     !     ", norm_diff_local = ", norm_diff_local, ", norm_x_local = ", norm_x_local
