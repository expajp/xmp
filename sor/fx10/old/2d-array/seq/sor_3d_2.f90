program sor_3d_2
  ...
  real(8) :: a_h_x_upper(sf, n), a_h_y_upper(sf, n), a_h_z_upper(sf, n) ! left-hand side
  real(8) :: a_h_x_lower(sf, n), a_h_y_lower(sf, n), a_h_z_lower(sf, n), a_diag ! left-hand side
  ...
  do j = 1, n-1
     do i = 1, sf
        if(mod(i,l-1) /= 0) a_h_x_upper(i, j) = 1.0d0/h_x**2 ! この条件で一番下の見切れ部分もきちんと0になる
        if(i <= (l-1)*(m-2)) a_h_y_upper(i, j) = 1.0d0/h_y**2 ! この条件でｒｙ
  ...
  do j = 1, n-1
     do i = 1, sf
        if(mod(i,l-1) /= 1) a_h_x_lower(i, j) = 1.0d0/h_x**2 ! この条件で一番上の見切れ部分もきちんと0になる
        if(i > l-1) a_h_y_lower(i, j) = 1.0d0/h_y**2 ! この条件でｒｙ
