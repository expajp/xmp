program jacobi_2d
  ...
  do
     ...
     x_new(1) = (b(1)-a_k(1)*x(l)-a_h(1)*x(2))/a_diag(1)

     do i = 2, mesh-1
           x_new(i) = b(i)-a_h(i)*x(i+1)-a_h(i-1)*x(i-1)
           if(i-l+1 >= 0) x_new(i) = x_new(i)-a_k(i-l+1)*x(i-l+1)
           if(i+l-1 <= mesh) x_new(i) = x_new(i)-a_k(i)*x(i+l-1)
           x_new(i) = x_new(i)/a_diag(i)
     end do

     x_new(mesh) = (b(mesh)-a_h(mesh-1)*x(mesh-1)-a_k(mesh-l+1)*x(mesh-l+1))/a_diag(mesh)
