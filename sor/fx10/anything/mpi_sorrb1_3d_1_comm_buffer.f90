! データ送信時に送受信用のバッファを使う実装、ただしバッファ使わないより遅い
program mpi_sorrb1_3d_1
  use mpi
  implicit none

  ! mesh
  integer, parameter :: l = 300, m = 300, n = 129
  ...
  double precision :: send_buffer(sf/2+1, 2), receive_buffer(sf/2+1, 2)
  ...
  do
     ...
     ! 送信用バッファの準備と受信用バッファのリセット
     do i = 1, sf/2+1
        send_buffer(i, 1) = x((start-1)+(i*2-1)) ! to left
        receive_buffer(i, 1) = 0.0d0
     end do

     do i = 1, sf/2+1
        if(i /= sf/2+1) send_buffer(i, 2) = x((goal-sf+1)+(i*2-1)) ! to right
        receive_buffer(i, 2) = 0.0d0
     end do
     ...
     ! to left
     call mpi_sendrecv(send_buffer(1, 1), sf/2+1, MPI_REAL8, leftnode, 100, &
          receive_buffer(1, 1), sf/2+1, MPI_REAL8, rightnode, 100, &
          MPI_COMM_WORLD, istat, ierr)

     ! to right
     call mpi_sendrecv(send_buffer(1, 2), sf/2+1, MPI_REAL8, rightnode, 100, &
          receive_buffer(1, 2), sf/2+1, MPI_REAL8, leftnode, 100, &
          MPI_COMM_WORLD, istat, ierr)

     ! 受信データの展開
     do i = 1, sf/2+1
        x(goal+(i*2-1)) = receive_buffer(i, 1)
     end do

     do i = 1, sf/2
        x((start-sf)+(i*2-1)) = receive_buffer(i, 2)
     end do
     ...
     ! 送信用バッファの準備と受信用バッファのリセット
     do i = 1, sf/2+1
        if(i /= sf/2+1) send_buffer(i, 1) = x((start-1)+(i*2)) ! to left
        receive_buffer(i, 1) = 0.0d0
     end do

     do i = 1, sf/2+1
        send_buffer(i, 2) = x((goal-sf-1)+(i*2)) ! to right
        receive_buffer(i, 2) = 0.0d0
     end do
     ...
     ! 受信データの展開
     do i = 1, sf/2
        x(goal+(i*2)) = receive_buffer(i, 1)
     end do

     do i = 1, sf/2+1
        x((start-sf-2)+(i*2)) = receive_buffer(i, 2)
     end do
