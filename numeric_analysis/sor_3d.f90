program sor_3d
  ...
  do
     ...
     x(1) = (b(1)-a_h_x(1)*x(2)-a_h_y(1)*x(l)-a_h_z(1)*x((l-1)*(m-1)+1)) * (omega/a_diag(1)) + (1-omega)*x(1)

     do i = 2, l-1
        x(i) = (b(i)-a_h_x(i-1)*x(i-1)-a_h_x(i)*x(i+1)-a_h_y(i)*x(i+l-1)-a_h_z(i)*x(i+(l-1)*(m-1))) &
                * (omega/a_diag(i)) + (1-omega)*x(i)
     end do

     do i = l, (l-1)*(m-1)
        x(i) = (b(i)-a_h_x(i-1)*x(i-1)-a_h_y(i-l+1)*x(i-l+1) &
                    -a_h_x(i)*x(i+1)-a_h_y(i)*x(i+l-1)-a_h_z(i)*x(i+(l-1)*(m-1))) &
                    * (omega/a_diag(i)) + (1-omega)*x(i)
     end do

     do i = (l-1)*(m-1)+1, mesh-(l-1)*(m-1)
        x(i) = (b(i)-a_h_x(i-1)*x(i-1)-a_h_y(i-l+1)*x(i-l+1)-a_h_z(i-(l-1)*(m-1))*x(i-(l-1)*(m-1)) &
                    -a_h_x(i)*x(i+1)-a_h_y(i)*x(i+l-1)-a_h_z(i)*x(i+(l-1)*(m-1))) &
                    * (omega/a_diag(i)) + (1-omega)*x(i)
     end do

     do i = mesh-(l-1)*(m-1)+1, mesh-l+1
        x(i) = (b(i)-a_h_x(i-1)*x(i-1)-a_h_y(i-l+1)*x(i-l+1)-a_h_z(i-(l-1)*(m-1))*x(i-(l-1)*(m-1)) &
                    -a_h_x(i)*x(i+1)-a_h_y(i)*x(i+l-1)) * (omega/a_diag(i)) + (1-omega)*x(i)
     end do

     do i = mesh-l+2, mesh-1
        x(i) = (b(i)-a_h_x(i-1)*x(i-1)-a_h_y(i-l+1)*x(i-l+1)-a_h_z(i-(l-1)*(m-1))*x(i-(l-1)*(m-1)) &
                    -a_h_x(i)*x(i+1)) * (omega/a_diag(i)) + (1-omega)*x(i)
     end do

     x(mesh) = (b(mesh)-a_h_x(mesh-1)*x(mesh-1)-a_h_y(mesh-l+1)*x(mesh-l+1) &
                    -a_h_z(mesh-(l-1)*(m-1))*x(mesh-(l-1)*(m-1))) * (omega/a_diag(mesh)) + (1-omega)*x(mesh)
