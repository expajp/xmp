program gs_3d
  ...
  do
     ...
     x(1) = (b(1)-a_h_x(1)*x(2)-a_h_y(1)*x(l)-a_h_z(1)*x((l-1)*(m-1)+1))/a_diag(1)

     do i = 2, mesh-1
           x(i) = b(i)-a_h_x(i)*x(i+1)-a_h_x(i-1)*x(i-1)
           if(i-l+1 > 0) x(i) = x(i)-a_h_y(i-l+1)*x(i-l+1) ! lower-triangle y
           if(i-(l-1)*(m-1) > 0) x(i) = x(i)-a_h_z(i-(l-1)*(m-1))*x(i-(l-1)*(m-1)) ! lower-triangle z
           if(i+l-1 <= mesh) x(i) = x(i)-a_h_y(i)*x(i+l-1) ! upper-triangle y
           if(i+(l-1)*(m-1) <= mesh) x(i) = x(i)-a_h_z(i)*x(i+(l-1)*(m-1)) ! upper-triangle z
           x(i) = x(i)/a_diag(i)
     end do
